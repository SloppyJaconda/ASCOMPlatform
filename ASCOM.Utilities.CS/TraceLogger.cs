...
namespace ASCOM.Utilities.CS
{
    ...
    public class TraceLogger : ITraceLogger, ITraceLoggerExtra, IDisposable
    {
        ...
        public TraceLogger() : base()
        {
            ...
            if (string.IsNullOrEmpty(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments)))
                g_DefaultLogFilePath = GetCommonProgramFilesx86() + TRACE_LOGGER_SYSTEM_PATH + TRACE_LOGGER_FILENAME_BASE + Format(DateTime.Now, TRACE_LOGGER_FILE_NAME_DATE_FORMAT);
            else
                g_DefaultLogFilePath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + TRACE_LOGGER_PATH + TRACE_LOGGER_FILENAME_BASE + Format(DateTime.Now, TRACE_LOGGER_FILE_NAME_DATE_FORMAT);
        ...
        ///     ''' </remarks>
        public new void LogMessage(string Identifier, string Message, bool HexDump)
        {
            string Msg = Message;
        ...
        ///     ''' Possible reasons for this are exceptions causing the normal flow of code to be bypassed or logic errors.</para>
        ///     ''' </remarks>
        public new void LogContinue(string Message, bool HexDump)
        {
            if (this.traceLoggerHasBeenDisposed)
                return; // Ignore this call if the trace logger has been disposed
        ...
        ///     ''' Possible reasons for this are exceptions causing the normal flow of code to be bypassed or logic errors.</para>
        ///     ''' </remarks>
        public new void LogFinish(string Message, bool HexDump)
        {
            if (this.traceLoggerHasBeenDisposed)
                return; // Ignore this call if the trace logger has been disposed
        ...
        ///     ''' </remarks>
        [ComVisible(false)]
        public new void LogMessage(string Identifier, string Message)
        {
            if (this.traceLoggerHasBeenDisposed)
                return; // Ignore this call if the trace logger has been disposed
        ...
        ///     ''' </remarks>
        [ComVisible(false)]
        public new void LogContinue(string Message)
        {
            if (this.traceLoggerHasBeenDisposed)
                return; // Ignore this call if the trace logger has been disposed
        ...
        ///     ''' </remarks>
        [ComVisible(false)]
        public new void LogFinish(string Message)
        {
            if (this.traceLoggerHasBeenDisposed)
                return; // Ignore this call if the trace logger has been disposed
        ...
        private void CreateLogFile()
        {
            ...
            {
                case "":
                case object _ when SERIAL_AUTO_FILENAME:
                    {
                        if (g_LogFileType == "")
                        ...
                        {
                            TodaysLogFilePath = g_LogFilePath + TRACE_LOGGER_FILENAME_BASE + Format(DateTime.Now, TRACE_LOGGER_FILE_NAME_DATE_FORMAT); // Append Logs yyyy-mm-dd to the user supplied log file
        ...
        ;
                        ...
                        catch (IOException ex)
                        {
                            ...
                            if (!ok)
                                throw new ASCOM.Utilities.Exceptions.HelperException("TraceLogger:CreateLogFile - Unable to create log file", ex);
                        }
            ...
            for (i = 1; i <= Strings.Len(p_Msg); i++)
            {
                ...
                switch (CharNo)
                {
                   ...
                   :
                    ...
                    case 12:
                    case 14 - 31:
                    case object _ when CharNo > 126 // All other non-printables should be translated
             :
                        {
                            l_Msg = l_Msg + "[" + Strings.Right("00" + Conversion.Hex(CharNo), 2) + "]";
                            break;
        ...
        private void GetTraceLoggerMutex(string Method, string Parameters)
        {
            ...
            catch (Threading.AbandonedMutexException ex)
            {
                ...
                {
                    LogEvent("TraceLogger", "AbandonedMutexException in " + Method + ": Throwing exception to application", EventLogEntryType.Warning, EventLogErrors.TraceLoggerMutexAbandoned, null/* TODO Change to default(_) if this is not a reference type */);
                    throw; // Throw the exception in order to report it
                }
                else
                {
                    LogEvent("TraceLogger", "AbandonedMutexException in " + Method + ": Absorbing exception, continuing normal execution", EventLogEntryType.Warning, EventLogErrors.TraceLoggerMutexAbandoned, null/* TODO Change to default(_) if this is not a reference type */);
            ...
            if (!GotMutex)
            {
                LogEvent(Method, "Timed out waiting for TraceLogger mutex in " + Method + ", parameters: " + Parameters, EventLogEntryType.Error, EventLogErrors.TraceLoggerMutexTimeOut, null/* TODO Change to default(_) if this is not a reference type */);
